...
async def http_redirect(ctx):
    ...
    try:
        ...
        return [
            Finding("http-redirect", C, Severity.PASS, code="no-cleartext", params={"host": host})
        ]
    ...
    if status in (301, 308) and to_https:
        return [Finding("http-redirect", C, Severity.PASS, code="ok", evidence=f"{status} → {loc}")]
    if status in (302, 303, 307) and to_https:
        ...
        return [
            Finding(
                "http-redirect", C, Severity.LOW, code="weak-redirect", evidence=f"{status} → {loc}"
            )
        ]
