...
_NEVER_BLESS = [
    ...
    (
        "CSP sans restriction de script",
        csp,
...
async def test_spf_plus_all_is_flagged(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_resolve",
        lambda name, rt: (
            ['"v=spf1 +all"'] if rt == "TXT" else (_ for _ in ()).throw(dns.resolver.NoAnswer())
        ),
    )
    spf = next(
        f for f in await dns_check(SimpleNamespace(host="example.com")) if f.check_id == "dns-spf"
    )
