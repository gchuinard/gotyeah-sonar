...
async def test_exposed_env_critical(make_ctx, make_response, fake_client_cls):
    env = make_response(
        200, headers={"content-type": "text/plain"}, text="SECRET_KEY=abc\nDB_PASSWORD=xyz\n"
    )
...
async def test_exposed_backup_no_false_positive_on_json_api(
    make_ctx, make_response, fake_client_cls
):
    ...
    json_200 = make_response(
        200, headers={"content-type": "application/json"}, text='{"data": [], "ok": true}'
    )
...
async def test_subresources_missing_nosniff(make_ctx, make_response, fake_client_cls):
    page = make_response(
        headers={"content-type": "text/html"}, text='<script src="/app.js"></script>'
    )
...
async def test_subresources_with_nosniff_pass(make_ctx, make_response, fake_client_cls):
    page = make_response(
        headers={"content-type": "text/html"}, text='<link rel="stylesheet" href="/style.css">'
    )
    asset = make_response(
...
async def test_subresources_cross_origin_ignored(make_ctx, make_response, fake_client_cls):
    ...
    ctx = make_ctx(
        response=page, url="https://example.com/", host="example.com", client=fake_client_cls()
    )
    out = await subresources(ctx)
