...
def mkresp(status=200, headers=None, text="", url="https://example.com/"):
    return httpx.Response(
        status, headers=headers, content=text.encode("utf-8"), request=httpx.Request("GET", url)
    )
...
async def _collect():
    ...
    await run(
        "hdr_csp_ok",
        H.csp(mkctx(response=mkresp(headers={"content-security-policy": "default-src 'self'"}))),
    )
    await run("hdr_hsts_absent", H.hsts(mkctx(response=mkresp(headers={}))))
    await run(
        "hdr_hsts_ok",
        H.hsts(mkctx(response=mkresp(headers={"strict-transport-security": "max-age=63072000"}))),
    )
    await run(
        "hdr_hsts_low",
        H.hsts(mkctx(response=mkresp(headers={"strict-transport-security": "max-age=100"}))),
    )
    ...
    await run(
        "hdr_nosniff_ok",
        H.nosniff(mkctx(response=mkresp(headers={"x-content-type-options": "nosniff"}))),
    )
    await run("hdr_nosniff_absent", H.nosniff(mkctx(response=mkresp(headers={}))))
    await run(
        "hdr_referrer_present",
        H.referrer(mkctx(response=mkresp(headers={"referrer-policy": "no-referrer"}))),
    )
    await run("hdr_referrer_absent", H.referrer(mkctx(response=mkresp(headers={}))))
    await run(
        "hdr_permissions_present",
        H.permissions(mkctx(response=mkresp(headers={"permissions-policy": "geolocation=()"}))),
    )
    await run("hdr_permissions_absent", H.permissions(mkctx(response=mkresp(headers={}))))
    await run(
        "hdr_disclosure_leak",
        H.disclosure(
            mkctx(response=mkresp(headers={"server": "nginx/1.2.3", "x-powered-by": "PHP/8.1"}))
        ),
    )
    await run(
        "hdr_disclosure_ok", H.disclosure(mkctx(response=mkresp(headers={"server": "nginx"})))
    )
    await run(
        "hdr_coop_present",
        H.coop(mkctx(response=mkresp(headers={"cross-origin-opener-policy": "same-origin"}))),
    )
    await run("hdr_coop_absent", H.coop(mkctx(response=mkresp(headers={}))))
    await run(
        "hdr_coep_present",
        H.coep(mkctx(response=mkresp(headers={"cross-origin-embedder-policy": "require-corp"}))),
    )
    await run("hdr_coep_absent", H.coep(mkctx(response=mkresp(headers={}))))
    await run(
        "hdr_corp_present",
        H.corp(mkctx(response=mkresp(headers={"cross-origin-resource-policy": "same-origin"}))),
    )
    ...
    await run(
        "hdr_cache_ok",
        H.cache(
            mkctx(response=mkresp(headers=[("set-cookie", "s=1"), ("cache-control", "no-store")]))
        ),
    )
    ...
    await run(
        "hdr_csp_permissive",
        H.csp(mkctx(response=mkresp(headers={"content-security-policy": "default-src *"}))),
    )
    await run(
        "hdr_xfo_allowall", H.xfo(mkctx(response=mkresp(headers={"x-frame-options": "ALLOWALL"})))
    )
    await run(
        "hdr_xfo_fa_wildcard",
        H.xfo(mkctx(response=mkresp(headers={"content-security-policy": "frame-ancestors *"}))),
    )
    await run(
        "hdr_hsts_disabled",
        H.hsts(mkctx(response=mkresp(headers={"strict-transport-security": "max-age=0"}))),
    )
    await run(
        "hdr_referrer_weak",
        H.referrer(mkctx(response=mkresp(headers={"referrer-policy": "unsafe-url"}))),
    )
    await run(
        "hdr_coop_weak",
        H.coop(mkctx(response=mkresp(headers={"cross-origin-opener-policy": "unsafe-none"}))),
    )
    ...
    await run(
        "ck_insecure",
        CK.cookies(mkctx(response=mkresp(headers=[("set-cookie", "sid=abc")]), url="https://x/")),
    )
    await run(
        "ck_samesite_none_insecure",
        CK.cookies(
            mkctx(
                response=mkresp(headers=[("set-cookie", "sid=abc; HttpOnly; SameSite=None")]),
                url="https://x/",
            )
        ),
    )
    ...
    await run(
        ...
        MX.mixed(
            mkctx(
                response=mkresp(headers={"content-type": HTML}, text="<img src='http://a/x'>"),
                url="http://x/",
            )
        ),
    )
    await run(
        ...
        MX.mixed(
            mkctx(
                response=mkresp(headers={"content-type": "application/json"}, text="{}"),
                url="https://x/",
            )
        ),
    )
        ...
        MX.mixed(
            mkctx(
                response=mkresp(
                    ...
                    text='<script src="http://a/x.js"></script><script src="http://b/y.js"></script>',
                ),
                url="https://x/",
            )
        ),
    ...
    await run(
        ...
        MX.mixed(
            mkctx(
                response=mkresp(
                    headers={"content-type": HTML}, text='<script src="http://a/x.js"></script>'
                ),
                url="https://x/",
            )
        ),
    ...
    await run(
        ...
        MX.mixed(
            mkctx(
                response=mkresp(headers={"content-type": HTML}, text='<img src="http://a/i.png">'),
                url="https://x/",
            )
        ),
    )
    await run(
        ...
        MX.mixed(
            mkctx(
                response=mkresp(headers={"content-type": HTML}, text='<img src="https://a/i.png">'),
                url="https://x/",
            )
        ),
    )
    ...
    await run(
        "cors_reflected",
        CR.cors(
            mkctx(client=FakeClient(default=mkresp(headers={"access-control-allow-origin": PROBE})))
        ),
    )
    await run(
        "cors_null",
        CR.cors(
            mkctx(
                client=FakeClient(default=mkresp(headers={"access-control-allow-origin": "null"}))
            )
        ),
    )
    await run(
        "cors_wildcard",
        CR.cors(
            mkctx(client=FakeClient(default=mkresp(headers={"access-control-allow-origin": "*"})))
        ),
    )
    ...
    exposed_routes = {
        ...
        ".DS_Store": mkresp(
            200, {"content-type": "application/octet-stream"}, "\x00\x00\x00\x01Bud1 stuff"
        ),
        "phpinfo.php": mkresp(200, {"content-type": "text/html"}, "<title>phpinfo()</title>"),
    }
    await run(
        "exposed_all", EX.exposed(mkctx(client=FakeClient(routes=exposed_routes), url="https://x/"))
    )
    ...
    exposed_extended_routes = {
        "wp-config.php": mkresp(
            200,
            {"content-type": "text/plain"},
            "<?php\ndefine('DB_NAME', 'wp');\ndefine('DB_PASSWORD', 's3cret');",
        ),
        ".aws/credentials": mkresp(
            200,
            {"content-type": "text/plain"},
            "[default]\naws_access_key_id = AKIAIOSFODNN7EXAMPLE\naws_secret_access_key = wJal",
        ),
        "terraform.tfstate": mkresp(
            200,
    ...
    await run(
        "exposed_extended",
        EX.exposed(mkctx(client=FakeClient(routes=exposed_extended_routes), url="https://x/")),
    )
    ...
    await run(
        ...
        SR.subresources(
            mkctx(
                ...
                host="example.com",
                client=FakeClient(routes={"/app.js": asset_no}),
            )
        ),
    )
    page_css = mkresp(headers={"content-type": HTML}, text='<link rel="stylesheet" href="/s.css">')
    asset_ok = mkresp(
        200, headers={"content-type": "text/css", "x-content-type-options": "nosniff"}
    )
    await run(
        "sr_protected",
        SR.subresources(
            mkctx(
                ...
                url="https://example.com/",
                host="example.com",
                client=FakeClient(routes={"/s.css": asset_ok}),
            )
        ),
    )
    page_x = mkresp(
        headers={"content-type": HTML}, text='<script src="https://cdn.autre.com/x.js"></script>'
    )
    await run(
        "sr_none",
        SR.subresources(
            mkctx(
                response=page_x, url="https://example.com/", host="example.com", client=FakeClient()
            )
        ),
    )
    await run(
    ...
    out["tls_cert_expired"] = [
        f.as_dict() for f in TLS._expiry_findings(_cert(not_after=_gmt(now - 86400)))
    ]
    out["tls_cert_soon"] = [
        f.as_dict() for f in TLS._expiry_findings(_cert(not_after=_gmt(now + 5 * 86400)))
    ]
    out["tls_cert_ok"] = [
        f.as_dict() for f in TLS._expiry_findings(_cert(not_after=_gmt(now + 200 * 86400)))
    ]
    out["tls_cert_not_yet"] = [
        f.as_dict()
        for f in TLS._expiry_findings(
    ...
    out["tls_key_weak"] = [
        f.as_dict() for f in TLS._cert_strength_findings(_mk_cert_der(1024, _hashes.SHA256()))
    ]
    out["tls_key_ok"] = [
        f.as_dict() for f in TLS._cert_strength_findings(_mk_cert_der(2048, _hashes.SHA256()))
    ]
    ...
    email_cases = [
        (
            "dns_dkim_present",
            DNS.dkim,
            {("google._domainkey.example.com", "TXT"): ['"v=DKIM1; k=rsa; p=ABC"']},
        ),
        ...
        (
            "dns_tlsrpt_present",
            DNS.tlsrpt,
            {("_smtp._tls.example.com", "TXT"): ['"v=TLSRPTv1; rua=mailto:t@x"']},
        ),
    ...
    for sid, cn, status, body in [
        ...
        try:
            await run(
                ...
                TK.takeover(
                    mkctx(
                        response=mkresp(status, {"content-type": "text/html"}, body),
                        host="blog.x.com",
                    )
                ),
            )
    ...
    find_routes = {
        ...
        "openapi.json": mkresp(
            200, {"content-type": "application/json"}, '{"openapi":"3.0.0","paths":{}}'
        ),
        "uploads/": mkresp(200, {"content-type": HTML}, "<title>Index of /uploads</title>"),
        ".well-known/security.txt": mkresp(
            200, {"content-type": "text/plain"}, "Contact: mailto:sec@x.com"
        ),
    }
    gql_resp = mkresp(
        200,
    ...
    page_f = mkresp(
        200, {"content-type": HTML}, '<script src="/app.js"></script>', url="https://x/"
    )
    await run(
        "leaks_findings",
        LK.leaks(
    ...
    await run(
        "leaks_clean",
        LK.leaks(mkctx(response=page_c, url="https://x/", host="x", client=_LeakFake({}, {}))),
    )
    ...
    await run(
        "methods_trace", MM.methods(mkctx(url="https://x/", client=_MethFake("GET, POST, TRACE")))
    )
    await run(
        "methods_dangerous",
        MM.methods(mkctx(url="https://x/", client=_MethFake("GET, POST, PUT, DELETE"))),
    )
    await run(
        "methods_ok",
        MM.methods(mkctx(url="https://x/", client=_MethFake("GET, POST, HEAD, OPTIONS"))),
    )
