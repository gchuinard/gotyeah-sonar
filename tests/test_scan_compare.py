...
def test_diff_resolved_new_persistent_and_delta():
    ...
    after = {
        ...
        "findings": [
            ...
            _f(
                "leak-srcmap", "exposed", "medium", "Source map", params={"path": "/a.js.map"}
            ),  # nouveau
    ...
    def ids(lst):
        return {(x["check_id"], x["code"]) for x in lst}
...
def test_extract_fixes_filters_and_interpolates_prompt():
    report = {
        ...
        "findings": [
            _f(
                "tech", "detected", "info", category="tech", params={"name": "nginx"}
            ),  # détection stack
            _f(
                "hdr-csp",
                "absent",
...
def test_detected_stack_mapping():
    assert (
        detected_stack(
            [_f("tech", "detected", "info", category="tech", params={"name": "Next.js"})]
        )
        == "nextjs"
    )
    assert (
        detected_stack(
            [_f("tech", "detected", "info", category="tech", params={"name": "Inconnu"})]
        )
        is None
    )
