...
def _resp(status=200, headers=None, text="", url="https://x/"):
    return httpx.Response(
        status, headers=headers or {}, content=text.encode(), request=httpx.Request("GET", url)
    )
...
def _ctx(page_html, client):
    return SimpleNamespace(
        url="https://x/", host="x", response=_resp(200, HTML, page_html), client=client
    )
...
async def test_all_findings():
    routes = {
        ...
        "/app.js": _resp(
            200, {"content-type": "application/javascript"}, "x\n//# sourceMappingURL=app.js.map"
        ),
    ...
    assert codes == {
        "leak-sourcemap": "exposed",
        "leak-swagger": "exposed",
        "leak-graphql": "introspection",
        "leak-autoindex": "listing",
...
async def test_root_probing_when_homepage_is_subpath():
    ...
    routes = {
        "https://x/openapi.json": _resp(
            200, {"content-type": "application/json"}, '{"openapi":"3.0.0"}'
        ),
    ...
    ctx = SimpleNamespace(
        url="https://x/en/",
        host="x",
        response=_resp(200, HTML, "<html></html>"),
        client=Fake(routes, {}),
    )
    out = await leaks(ctx)
    codes = {f.check_id: f.code for f in out}
...
async def test_all_clean():
    ...
    codes = {f.check_id: f.code for f in out}
    assert codes == {
        "leak-sourcemap": "clean",
        "leak-swagger": "clean",
        "leak-graphql": "clean",
        "leak-autoindex": "clean",
        "leak-securitytxt": "missing",
    }
